...
export function UploadMaterialModal({ children, workspaceId }: UploadMaterialModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="bg-[#18191A] border border-[#333] text-white  w-[540px] max-h-[800px] mx-auto p-0 overflow-hidden rounded-2xl shadow-2xl overflow-y-auto overflow-x-hidden" style={{ marginLeft: '300px' }}>
                {/* Nav Tabs */}
                ...
                <div className="flex flex-col items-start justify-start px-12 pb-8 pt-4 w-full h-full">
                    {activeTab === "Materials" && (
                        <>
                        {/* Files List */}
                        {workspace?.files?.pdfFiles?.length > 0 ? (
                            <div className="flex justify-between flex-wrap gap-2 mx-auto">
                                {workspace.files.pdfFiles.map((file: { id: string; filePath: string; fileName: string; size: string }) => (
