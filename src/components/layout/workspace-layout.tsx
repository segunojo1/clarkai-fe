...
const WorkspaceLayout = ({ children }: WorkspaceLayoutProps) => {
  ...
  return (
    <div className={`flex h-screen w-full transition-all duration-300`}>
      ...
      <div
        className={`flex-1 flex dark:bg-[#1a1a1a] bg-[#FAFAFA] text-white w-full justify-end !max-w-[calc(100vw)]`}
      >
        {!workspaceOpen && (
        ...
        <div className="flex w-full min-h-screen">
          <div className="flex-1 min-w-0 flex">
            ...
            {isFlashcardModalOpen && (
              <div className="flex-shrink-0 w-[452px] max-w-[542px] min-w-[360px]">
                <FlashcardPanel
        ...
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-2">
            <Globe
              width={19}
              height={19}
              color={workspace.tag ? workspace.tag : "#99a1af"}
            />
                  ...
                  <>
                    {(workspaceData?.files?.pdfFiles || []).length > 0 && (
                      <div className="space-y-1">
                        ...
                        {(workspaceData?.files?.pdfFiles || []).map(
                          (material) => (
                            <Link
                              target="_blank"
                              href={material.filePath}
                              key={material.id}
                              className="cursor-pointer relative pl-6 flex items-center gap-2"
                            >
                              ...
                              />
                              <div>
                                {material.fileName.length > 10
                                  ? `${material.fileName.slice(0, 10)}...pdf`
                                  : material.fileName}
                              </div>
                            </Link>
                          ),
                    ...
                    {(workspaceData?.files?.imageFiles || []).length > 0 && (
                      <div className="space-y-1">
                        ...
                        {(workspaceData?.files?.imageFiles || []).map(
                          (material) => (
                            <Link
                              target="_blank"
                              href={material.filePath}
                      ...
                      <div className="space-y-1">
                        {(workspaceData?.quizzes || []).map((quiz) => (
                          <Link
                            href={`/quiz/${quiz.id}`}
                            key={quiz.id}
                            className="relative pl-3 flex items-center gap-2"
                          >
