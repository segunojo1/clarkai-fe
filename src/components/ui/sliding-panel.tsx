...
export function SlidingPanel({
  ...
  const [generatedQuiz, setGeneratedQuiz] = useState<{ id: string } | null>(
    null,
  );
  ...
  const materials = [
    ...
    ...((
      selectedWorkspace as WorkspaceWithFiles
    )?.workspace?.files?.pdfFiles?.map((file: WorkspaceFile) => ({
      id: file.id,
      name: file.fileName,
    })) || []),
  ];
    ...
    if (currentStep === 1) {
      ...
      setFormData((prev) => ({
        ...
        step3: {
          ...
          numQuestions: prev.step2.numQuestions || 5,
          timeLimit:
            prev.step1.isTimed === "yes"
              ? Number(prev.step1.timePerQuestion)
              : 30,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const numQuestions =
        formData.step3.numQuestions || formData.step2.numQuestions || 5;
      const difficulty = formData.step2.difficulty.toLowerCase() as
        | "easy"
        | "medium"
        | "hard";
      const isTimed = formData.step1.isTimed === "yes";
      const duration = isTimed
        ? Number(formData.step1.timePerQuestion)
        : undefined;

      // Determine mode and file ID
      const source =
        formData.step3.source || formData.step1.materialId || "all";
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return (
          <div className="p-4">
            <h3 className="text-3xl font-bold dark:text-white text-black mb-6">
              Let&apos;s Build a Quiz!
            </h3>
      ...
      case 2:
        return (
          <div className="p-4">
            ...
            <h3 className="text-3xl font-bold dark:text-white text-black mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 w-[341px]">
              ...
              <div>
                ...
                >
                  <option value="">
                    If material is selected, select the focus
                  </option>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="dark:bg-[#1E1E1E] bg-white p-4 rounded-lg border border-[#333]">
                    <h4 className="text-[#FF3D00] text-sm font-medium mb-3">
                      Quiz Preview
                    </h4>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-black dark:text-white capitalize">
                          {formData.step3.questionType === "mcq"
                            ? "Multiple Choice"
                            : formData.step3.questionType === "true_false"
                              ? "True/False"
                              : formData.step3.questionType === "short_answer"
                                ? "Short Answer"
                                : formData.step3.questionType === "mixed"
                                  ? "Mixed"
                                  : "N/A"}
                        </span>
                      </div>
                      ...
                      {formData.step3.focusArea && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-black dark:text-white capitalize">
                            {formData.step3.focusArea === "concepts"
                              ? "Key Concepts"
                              : formData.step3.focusArea === "examples"
                                ? "Examples & Applications"
                                : formData.step3.focusArea === "theorems"
                                  ? "Theorems & Proofs"
                                  : ""}
                          </span>
                        </div>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-black dark:text-white capitalize">
                          {formData.step2.difficulty.toLowerCase()}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-sm text-black dark:text-gray-400 text-center py-2">
                    Review your quiz settings. Click &quot;Generate Quiz&quot;
                    to create your quiz.
                  </div>
                </div>
        ...
        return (
          <div className="p-6 flex flex-col items-center justify-center h-full">
            ...
            <p className="text-black dark:text-gray-400 text-center">
              Please wait while we prepare your quiz. This may take a moment.
            </p>
          </div>
        );
      case 5:
                ...
                <li className="flex justify-between items-center">
                  <div className="flex items-center space-x-2 text-black dark:text-gray-400">
                    ...
                    <span className="text-black dark:text-white">
                      {formData.step3.source === "all"
                        ? "All Materials"
                        : materials.find((m) => m.id === formData.step3.source)
                            ?.name || "Unknown"}
                    </span>
                  </div>
                  <button
